...
class MysqlProfileRepository {
    ...
    async findByUsername(username) {
        const [rows] = await this.pool.query('SELECT * FROM analyzed_profiles WHERE username = ?', [username]);
        if (rows.length === 0) {
            return null;
        }
    ...
    async list(filters) {
        ...
        const [countRows] = await this.pool.query(`SELECT COUNT(*) AS total FROM analyzed_profiles ${whereClause}`, params);
    ...
    async save(profile) {
        ...
        try {
            ...
            const profileValues = [
                ...
                profile.followers,
                profile.following,
                (0, dates_1.toUtcDate)(profile.accountCreatedAt),
                ...
                (0, dates_1.toUtcDate)(profile.analyzedAt),
                (0, dates_1.toUtcDate)(profile.analyzedAt),
            ];
        ...
        `, profileValues);
            ...
            const repositoryRows = profile.rawRepos.map((repo) => [
                ...
                repo.watchers_count,
                repo.size,
                (0, dates_1.toUtcDate)(repo.pushed_at),
                (0, dates_1.toUtcDate)(repo.updated_at),
                repo.fork ? 1 : 0,
                repo.archived ? 1 : 0,
                JSON.stringify(repo),
            ]);
    ...
    async deleteByUsername(username) {
        ...
        try {
            await connection.beginTransaction();
            const [rows] = await connection.query('SELECT id FROM analyzed_profiles WHERE username = ?', [username]);
            if (rows.length === 0) {
                await connection.rollback();
                return false;
            }
    ...
    async findProfileIdByUsername(username, connection) {
        const [rows] = await connection.query('SELECT id FROM analyzed_profiles WHERE username = ?', [username]);
        if (rows.length === 0) {
    ...
    async mapStoredAnalysis(profileRow) {
        const [repoRows] = await this.pool.query('SELECT * FROM analyzed_profile_repositories WHERE profile_id = ? ORDER BY stars DESC, github_repo_id DESC', [profileRow.id]);
        const [languageRows] = await this.pool.query('SELECT * FROM analyzed_profile_languages WHERE profile_id = ? ORDER BY stars DESC, repo_count DESC, language_name ASC', [profileRow.id]);
        ...
        const profile = {
            ...
            activityData: this.parseJsonField(profileRow.activity_data_json, []),
            analyzedAt: new Date(profileRow.analyzed_at).toISOString(),
            insights: this.parseJsonField(profileRow.insights_json, {}),
            activitySummary: this.parseJsonField(profileRow.activity_summary_json, {}),
            githubId: Number(profileRow.github_id),
            githubProfileUrl: profileRow.github_profile_url,
            rawProfile: this.parseJsonField(profileRow.raw_profile_json, {}),
            rawRepos,
            rawEvents: this.parseJsonField(profileRow.raw_events_json, []),
        };
        return profile;
    }
