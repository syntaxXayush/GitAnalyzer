...
exports.ProfileAnalysisService = void 0;
const normalize_username_1 = require("../utils/normalize-username");
...
class ProfileAnalysisService {
    repository;
    githubService;
    constructor(repository, githubService) {
        this.repository = repository;
        this.githubService = githubService;
    }
    async analyze(username, forceRefresh = false) {
        const normalized = (0, normalize_username_1.normalizeUsername)(username);
        if (!normalized) {
            throw new app_error_1.ValidationError('GitHub username is required');
        }
        if (!forceRefresh) {
    ...
    async findByUsername(username) {
        const profile = await this.repository.findByUsername((0, normalize_username_1.normalizeUsername)(username));
    ...
    async delete(username) {
        return this.repository.deleteByUsername((0, normalize_username_1.normalizeUsername)(username));
    }
    buildAnalysis(user, repositories, events) {
        const repoCount = repositories.length;
        const totalStars = repositories.reduce((sum, repo) => sum + repo.stargazers_count, 0);
        const totalForks = repositories.reduce((sum, repo) => sum + repo.forks_count, 0);
        const analyzedAt = new Date().toISOString();
        const accountCreatedAt = new Date(user.created_at).toISOString();
        const accountAgeYears = (0, dates_1.yearsBetween)(new Date(user.created_at), new Date(analyzedAt));
        ...
        const mostStarredRepo = repositories.reduce((best, repo) => {
        ...
        const monthlyBuckets = (0, dates_1.monthLabels)(12);
        ...
        const scoring = (0, score_1.buildDeveloperScore)({
            user,
            repositories,
            languages,
            activitySummary,
...
exports.ProfileAnalysisService = ProfileAnalysisService;
