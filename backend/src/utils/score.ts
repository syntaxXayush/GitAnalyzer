import { ActivitySummary, GitHubRepository, GitHubUser, LanguageInsight, ProfileInsights, TopRepositoryInsight } from '../domain/profile';

function clamp(value: number, min: number, max: number): number {
  return Math.min(max, Math.max(min, value));
}

export function buildDeveloperScore(input: {
  user: GitHubUser;
  repositories: GitHubRepository[];
  languages: LanguageInsight[];
  activitySummary: ActivitySummary;
}): { score: number; level: string; insights: ProfileInsights } {
  ...
  const mostStarredRepo = input.repositories.reduce<TopRepositoryInsight | null>((best, repo) => {
  ...
  return {
    ...
    insights: {
      ...
      opportunities,
      activitySignal:
        input.activitySummary.recentActivityScore > 140
